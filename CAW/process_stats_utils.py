...
def generate_results_stats(res_df, EVAL_MODE, verbose=False):
    ...
    try:
        stats_dic['GMAUC'] = np.sqrt(((stats_dic['new_e_PRAUC'] - (float(stats_dic['n_pos_new']) /
                                                               float((stats_dic['n_pos_new'] + stats_dic[
                                                                     'n_neg_new'])))) /
...
def gen_avg_perf(all_stats_df_filename, avg_stats_df_filename, DATA, EVAL_MODE, LP_MODE, TR_NEG_SAMPLE, TS_NEG_SAMPLE):
    ...
    all_cols_w_values = ['hist_e_PRAUC', 'hist_e_AUC', 'hist_e_AP',
                         'new_e_PRAUC', 'new_e_AUC', 'new_e_AP', 
                         'GMAUC', 
    ...
    if EVAL_MODE == 'SNAPSHOT':
        ...
        setting_res = stats_df.loc[((stats_df['DATA'] == DATA) & (stats_df['LP_MODE'] == LP_MODE) &
                                (stats_df['TR_NEG_SAMPLE'] == TR_NEG_SAMPLE) 
                                & (stats_df['EVAL_MODE'] == EVAL_MODE)),
                                all_cols_w_values]
