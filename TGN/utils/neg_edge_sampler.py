...
class NegativeEdgeSampler(object):
    ...
    def sample_rnd_hitsK(self, size, cur_sources, cur_destinations):
        """
        sample 'size' negative edges for each positive edge
        """
        replace = False
        distict_pos_e = {}
        for src, dst in zip(cur_sources, cur_destinations):
            if (src, dst) not in distict_pos_e:
                distict_pos_e[(src, dst)] = 1
        # negative edges selection
        ...
        for src in cur_sources:
            ...
            while num_selected_smp_rnd < size:
                ...
                rnd_neg_dst = self.dst_list_distinct[current_rnd_dst_index][0]
                if (src, rnd_neg_dst) not in distict_pos_e:
                    neg_rnd_source.append(src)
                    neg_rnd_dest.append(rnd_neg_dst)
