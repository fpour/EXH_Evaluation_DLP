...
class RandEdgeSampler_TGAT(object):
...
class NegativeEdgeSampler(object):
    ...
    There are four different ways of negative edge sampling:
    - rnd: Random negative edge sampling with the colision check
    - haphaz_rnd: Original random negative sampling without any check
    - hist: Historical negative sampling 
    - induc: Inductive negative sampling
    """

    def __init__(self, src_list, dst_list, ts_list, last_ts_train_val, NS='hist', seed=None, rnd_sample_ratio=1.0):
        """
    ...
    def sample_induc_NE(self, size, current_split_start_ts, current_split_end_ts):
        ...
        NOTE:
          ...
          2. All edges are selected according to the inductive NS setting, unless we don't have enough inductive negative edges
        """
        history_e_dict = self.get_edges_in_time_interval(self.ts_last_before_test, current_split_start_ts)
        current_split_e_dict = self.get_edges_in_time_interval(current_split_start_ts, current_split_end_ts)
