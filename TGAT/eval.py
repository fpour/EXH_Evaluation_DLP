...
def eval_link_pred(model, sampler, data, stats_filename=None, batch_size=32, n_neighbors=20):
    """
    ...
    with torch.no_grad():
        ...
        for k in range(num_test_batch):
            ...
            e_idx = min(num_test_instance - 1, s_idx + TEST_BATCH_SIZE)
            if s_idx == e_idx:
                print(f"DEBUG: Not enough Positive edges in the batch {k}! [len(data.sources[s_idx:e_idx]): {len(data.sources[s_idx:e_idx])}]")
                continue
            ...
            if len(dst_l_fake) > 1:
                ...
                neg_prob = model.contrast_modified(src_l_fake, dst_l_fake, ts_l_cut, n_neighbors)
                ...
                print(f"DEBUG: Not enough Negative edges in the batch {k}! [len(dst_l_fake): {len(dst_l_fake)}]")
...
def eval_link_pred_one_snapshot(model, snap_data, stats_filename=None, batch_size=32, n_neighbors=20):
    """
    Evaluate the link prediction task
    """
    if stats_filename is not None:
    ...
    with torch.no_grad():
        ...
        for p_b_idx in range(NUM_TEST_BATCH_POS):
            ...
            if len(pos_src_batch) > 1:
                pos_prob = model.contrast_modified(pos_src_batch, pos_dst_batch, pos_ts_batch, n_neighbors)
                ...
                print(f"DEBUG: no Positive edges in batch P-{p_b_idx}! [len(pos_src_batch): {len(pos_src_batch)}]")
            ...
            for n_b_idx in range(NUM_NEG_BATCH_PER_POS_BATCH):
                ...
                if len(neg_src_batch) > 1:
                    neg_prob = model.contrast_modified(neg_src_batch, neg_dst_batch, neg_ts_batch, n_neighbors)
                    ...
                    print(f"DEBUG: no Negative edges in batch P-{p_b_idx}_N-{n_b_idx}! [len(neg_src_batch): {len(neg_src_batch)}]")
