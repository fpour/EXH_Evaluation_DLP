...
def get_args():
    ...
    parser.add_argument('--ts_neg_sample', type=str, default='hist', choices=['rnd', 'haphaz_rnd', 'hist', 'induc'],
                        help='Strategy for the edge negative sampling.')
...
def main():
    """
    execution command:
        python process_stats.py --seed 123 --n_runs 5 --tr_neg_sample "haphaz_rnd" --ts_neg_sample "haphaz_rnd" --eval_mode STD --lp_mode trans --data wikipedia 
    """
    print("INFO: process_stats.py")
    args, _ = get_args()
    DATA = args.data
    N_RUNS = args.n_runs
    OPT = args.opt
    ...
    if OPT == 'avg':
        ...
        invoke_stats_generation(partial_path, partial_name, stats_filename, full_data, DATA, EVAL_MODE, 
